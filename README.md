...
| **Fee waiver at the gate** | waiver-aware (`fee_gate_mode`) | If the user's declared spend crosses the card's waiver threshold, the fee is treated as ₹0 at the gate too, not just in NAV — a card that's genuinely free at that spend level shouldn't disappear for stating a strict budget. Switch to `hard_sticker` for a stricter "budget means budget" mode that ignores waiver reachability. |
